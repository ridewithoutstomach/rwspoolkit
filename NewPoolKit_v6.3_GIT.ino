/* *****************************************************************
  ...
  *******************************************************************/
// v6.2 - Login-PIN wieder aenderbar ueber Webseite (/pw.htm)
...
#include <DNSServer.h>          // v6.2: Captive Portal
#include <NTPClient.h>
...
// v6.2: ein Hostname fuer WiFi + OTA (konfigurierbar ueber /wifi.htm)
char hostname[60] {"RWSPoolkit"};

// v6.2: DNS-Server fuer Captive Portal im AP-Modus
...
HTTPClient http;
ESP8266WebServer server(80); //Server on port 80   // Webserver
...
// v6.2: Login-PIN aus LittleFS laden (falls vorhanden, sonst bleibt Default)
read_lizenz();
read_thing();
...
polling = true;
Serial.println("v6.2: polling=TRUE (always on)");
...
//  Wifi_Seq.reset();                       //initialize the sequencers
  ...
  // v6.2: Hostname aus WiFi-Config (wird auch fuer OTA verwendet)
...
void loop() {
  ...
  // OTA
  ArduinoOTA.handle();
  dnsServer.processNextRequest();  // v6.2: Captive Portal (nur aktiv im AP-Modus)
  reStation();
...
// v6.2: Fallback mit .local-Suffix fuer mDNS-Aufloesung
