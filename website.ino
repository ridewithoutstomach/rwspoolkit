...
void handleForm() {
  ...
  // ---------------------------------------------Pool Abfrage Anfang
    ...
    if ( login ) {
      ...
      delay(100);
      read_pool();
      pumpe_fail_time = 0;  // v6.2: Fail-Cache zuruecksetzen nach Hostname-Aenderung
      String message;
      addTop(message);
...
// v6.2: handleRoot - AP-Modus: WiFi-Setup, sonst Login/Dashboard
void handleRoot() {

    // v6.2: AP-Modus -> WiFi-Setup Captive Portal
    if ( ap_mode ) {
      String message;
